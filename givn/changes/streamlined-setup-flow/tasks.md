...
- [ ] Configure `./run-tests.sh` and `./run-tests.sh --e2e` as the verification commands; keep `Cucumber::fail_on_skipped()` strictness; create/register `tests/steps/streamlined_setup_steps.rs` and `tests/steps/streamlined_setup_e2e_steps.rs`; prove an undefined or `unimplemented!()` step exits non-zero. Evidence: command/output:
- [ ] Confirm the runner executes permanent `givn/specs/**` and active change `specs/**`, and confirm the E2E tag filter is a strict subset. Evidence: command/output:
- [ ] Setup production files changed: `src/main.rs`, `src/setup.rs`, `src/config/mod.rs`, `src/config/types.rs`, `src/models/list.rs`, `src/models/dialog.rs`, `src/provider/setup.rs`, `src/shell_completion.rs`, `src/shell_shortcut.rs`, and any narrowly required support modules. Evidence: compile/output:
- [ ] Setup commit hash: 
