...
| 9 | 09 architecture-decisions | No | No ADR qualifies: the contract is givn process guidance owned by the change design, not a project architecture decision; no existing ADR is refined. | |
...
The existing ADR bodies live at `docs/adr/` (chapter 09 register links there);
this maintenance change creates no new ADR and refines no existing one. The
result contract is owned by the change design and givn process guidance.
