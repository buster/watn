...
| Structured review response | Read the review-mode provider response tolerantly (repair literal control characters, recover a complete provider-written command from a malformed or truncated payload, refuse to treat a review-shaped payload as command text), then validate it, retain model-written Stage purposes, reject stale or mismatched purpose data, carry the Purpose reason, and expose `ready`, `loading`, or `purpose-unavailable` status |
| Review diagnostics | Capture the most recent unusable provider response in the state file, name the saved path after the review surface closes, and print the raw provider response under `-v`; a capture failure warns and never changes the review outcome |
...
| Review preference | Resolve the per-invocation review override over persisted `[review]` configuration and the built-in enabled default; a review switch without a request only persists the requested value; disabled or ineligible mode follows the existing output path |
