...
| Review card renderer | Render the framed card (header, command, stage position, selected stage and purpose, decision hints with emphasized shortcut letters, model chooser tiers/search/picks with the active model marked, the nested-syntax stage marker, and key hints) as the only review surface; color is a property of the terminal, not an adapter choice |
