...
| Review card renderer | Render the framed card (header, command, stage position, selected stage and purpose, decision hints with emphasized shortcut letters, model chooser rows, key hints) as the only review surface; color is a property of the terminal, not an adapter choice |
