...
| Review surface | Own review eligibility, buffered Candidate state, Command flow derivation, Purpose status, direct decision keys, Candidate replacement through the in-card model chooser, acceptance/cancellation outcomes, and transient inline terminal rendering; never execute or write review-surface text to stdout |
| Review card renderer | Render the framed card (header, command, stage position, selected stage and purpose, decision hints with emphasized shortcut letters, model chooser tiers/search/picks with the active model marked, an amber ellipsis on stages it does not decompose, and key hints) as the only review surface; color is a property of the terminal, not an adapter choice |
