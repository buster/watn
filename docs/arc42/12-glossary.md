...
| Amount | The money the provider billed for one completed request: the usage the provider reported, paired with the recorded per-million price that applies to the request — the price of the model the provider reported, or, when that model has no recorded price, the price of the model the request was sent with — displayed in cents with one significant digit and never more than four decimal places, whole cents from one cent on. Anti-terms: not Pricing or Catalog price (the per-million rate), not a prediction of future spend, not a review-session total |
...
| Simple review view | The default reduced presentation of the Review surface: model short name with the Amount billed for the request, the Stage stack with the selected-stage marker, the selected Stage purpose below the stack, and only the accept, stage-navigation, view-toggle, and Escape decisions in its hints; edit, reject, and disable remain available and promote or act without changing the minimal presentation |
...
| Model short name | The final `/`-separated identifier segment of the configured model with any leading `~` and `:` variant suffix removed, shown in the simple review view frame; when the header cannot hold both the Model short name and the Amount, the name is shortened first |
| Model label | The review-surface element naming the model that produced the surface: the Model short name in the simple view, the tier and provider-qualified model in the detailed view. The Amount is shown with it, and no Amount appears without one. Anti-terms: not the model the provider reported, which the Amount is priced from first, and not the model the developer configured, which may carry a tier rather than a name |
...
| Release note | The one complete, user-facing sentence a change authors in its proposal and carries as the message subject of every revision it records. Anti-terms: not the scenario title (the implementation's unit of work), not a per-revision summary |
| Changelog | The release-facing list of what changed for the developer: one entry per change, taken from the change's Release note, limited to features, bug fixes, performance changes, reverts, and breaking changes. Anti-terms: not the revision log, not the full history |
