...
| Finish config commit is secure and atomic | Setup reads without side effects and commits supported configuration once at Finish using a restrictive temporary file, flush/sync, atomic rename, and Unix mode `0600` |
...
| First-run detection is physical | The existence of the configured path is checked before parsing or readiness; a comment-only file counts as existing and a missing path is never initialized as a side effect |
| Provider selection is explicit in setup | Provider/model override flags and `WATN_PROVIDER`/`WATN_MODEL` overlays are removed; setup offers OpenRouter, OpenAI, and Custom choices |
...
| Credential source is authoritative | A literal saved key or complete saved `${VARIABLE}` reference cannot be replaced by environment fallback; first-run discovery reads only an allowlist and exposes names/presence, never values |
