...
- **Shell integration safety** — Optional setup, safe startup-file ownership, non-evaluating line replacement, and request preservation
  ...
  - QS-051: The widget preserves the buffer on empty, failed, or empty results
   ...
   - QS-067: Eligible review displays a small inline review card after the existing progress line and a complete Candidate reaches `[DONE]`, without entering the alternate screen; the card opens on the command flow and presents intent, command, selected stage and purpose, direct decision hints with accept emphasized, and key hints distinguishably, renders monochrome without color support, and can persist its own disable
