...
- **Shell integration safety** — Optional setup, safe startup-file ownership, non-evaluating line replacement, and request preservation
  ...
  - QS-051: The widget preserves the buffer on empty, failed, or empty results
   ...
   - QS-067: Eligible review displays a small inline review card after the existing progress line and a complete Candidate reaches `[DONE]`, without entering the alternate screen; the card opens on the command flow and presents intent, command, selected stage and purpose, direct decision hints with emphasized shortcut letters and accept emphasized, and key hints distinguishably, renders monochrome without color support, and can persist its own disable
   ...
   - QS-070: The command flow is active when the card opens; arrows move stages; Enter or `a` accepts, `e` edits, `r` rejects, `c` or Escape cancels; direct editing moves the insertion point with arrows and Home/End and commits with Enter and discards with Escape
...
| QS-067 | Shell integration / Usability | User invokes eligible Ctrl-W review with the panel enabled, disabled, or color-incapable | Existing progress appears first; after `[DONE]` a small transient inline review card follows without alternate-screen sequences; it opens on the command flow and shows intent, command, selected stage and purpose, decision hints with emphasized shortcut letters and accept emphasized, and key hints, renders monochrome without color, and the `d` decision persists the disable |
...
| QS-070 | Shell integration / Usability | User navigates and edits the review surface | The card opens on the command flow; arrows move stages; Enter or `a` accepts, `e` edits, `r` rejects, `c`/Escape cancels; the editor moves its insertion point with arrows and Home/End; editor Enter commits and editor Escape discards |
