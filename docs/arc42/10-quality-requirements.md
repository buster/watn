...
- **Shell integration safety** — Optional setup, safe startup-file ownership, non-evaluating line replacement, and request preservation
  ...
  - QS-051: The widget preserves the buffer on empty, failed, or empty results
   ...
   - QS-067: Eligible review displays a small inline review card after the existing progress line and a complete Candidate reaches `[DONE]`, without entering the alternate screen; the card opens in a simple view that names the model short name with the Amount billed for the request, stacks the command flow with separators and a selected-stage arrow, and shows the selected stage's marked purpose below the stack; `d` or `?` switches to the detailed view and back, the detailed view adds Intent and the full decision hints and keeps the Amount at its model context, renders the same command block as the simple view with a blank row before the marked purpose, and shows no redundant flow, stage, or support labels, the card renders monochrome without color support, and `D` persists the disable while releasing the current Candidate
   ...
   - QS-070: The command flow is active when the card opens; arrows move stages in both views; Enter accepts, `e` edits in the detailed view, `r` rejects in the detailed view, `c` or Escape cancels, and `d`/`?` switches the view; direct editing moves the insertion point with arrows and Home/End and commits with Enter and discards with Escape
   - QS-071: A review card that cannot open preserves the original input and releases no Candidate
    - QS-072: Structured review responses retain model-written Stage purposes; repaired, fenced, prose-wrapped, unknown-status, multiline, compound-split, invalid, truncated, or command-only responses keep the provider-written command reviewable with purpose-unavailable when no matching purposes exist, and a review-shaped payload is never shown as the command
     ...
     - QS-080: An unusable review or explanation response names its Purpose reason in the review surface, is captured best-effort in the overwritten state file with the saved path named on stderr after the surface closes, and is printed raw on stderr under `-v`; stdout stays free of diagnostics
...
| QS-067 | Shell integration / Usability | User invokes eligible Ctrl-W review with the panel enabled, disabled, or color-incapable | Existing progress appears first; after `[DONE]` a small transient inline review card follows without alternate-screen sequences; it opens in the simple view with the model short name and the Amount billed for the request, the stage stack with separators and selected-stage arrow, and the marked purpose below the stack; `d`/`?` switches to the detailed view and back with Intent, the same Amount at its model context, the same command block as the simple view, a blank row before the marked purpose, and no redundant flow, stage, or support labels, renders monochrome without color, and `D` persists the disable while releasing the current Candidate |
...
| QS-070 | Shell integration / Usability | User navigates and edits the review surface | The card opens on the command flow; arrows move stages in both views; Enter accepts, `e` edits in the detailed view, `r` rejects in the detailed view, `c`/Escape cancels, and `d`/`?` switches the view; the editor moves its insertion point with arrows and Home/End; editor Enter commits and editor Escape discards |
| QS-074 | Shell integration / Recovery | User rejects a candidate and chooses another model from the chooser | The chooser labels its tier keys and search field, marks the current model, and pre-selects the first suggestion without blocking; choosing a tier or model regenerates and returns to the card; a failed or unavailable generation keeps the previous candidate and reports the failure; no candidate is released by the chooser |
| QS-071 | Shell integration / Portability | Review card cannot open | The original input is preserved, the review is `Unavailable`, and no Candidate is released |
| QS-072 | Shell integration / Correctness | Provider returns ready, delayed, repaired, fenced, prose-wrapped, unknown-status, multiline, command-only, invalid, truncated, missing-command, or stale review response data | Ready data displays exact Stage text and model-written purposes; valid delayed data displays loading then purposes; repaired or fenced or prose-wrapped data is recognized; matching stage purposes survive an unknown status word, a multiline command, and a compound provider split that provably covers the command; a truncated payload with a complete command shows that command with purpose-unavailable and the incomplete reason; a malformed payload with an unescaped quotation mark in the command still shows the provider-written command; mismatched or missing purposes show purpose-unavailable with a Purpose reason; a review-shaped payload is never shown as the command; a payload without a usable command shows unavailable and releases nothing; purpose text is never invented |
...
| QS-077 | Command explanation / Recovery | A failed explanation request, or a response whose stages do not cover the command | The card shows the developer's command with purpose-unavailable and the model's command text never replaces the developer's; a failed request is named on stderr and the invocation exits with the mapped status after the card closes; a non-covering response is named on stderr, shows the Purpose reason in the card, is captured in the state file, and exits 0 |
| QS-080 | Shell integration and command explanation / Observability and Recovery | Provider returns an unusable response during review or explanation, with and without `-v`, and the state directory is unwritable | The card names the Purpose reason; the raw response is saved to the overwritten state file and its path is named on stderr after the surface closes; `-v` prints the raw response to stderr; an unwritable state file warns and the review stays reviewable; stdout contains no diagnostic |
