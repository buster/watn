...
- **Shell integration safety** — Optional setup, safe startup-file ownership, non-evaluating line replacement, and request preservation
  ...
  - QS-051: The widget preserves the buffer on empty, failed, or empty results
   ...
   - QS-067: Eligible review displays a small inline review surface after the existing progress line and a complete Candidate reaches `[DONE]`, without entering the alternate screen; the enhanced card presents intent, command, selected stage and purpose, focus regions, actions, and key hints distinguishably, and falls back to the plain panel when disabled or color-incapable
...
| QS-067 | Shell integration / Usability | User invokes eligible Ctrl-W review with the panel enabled, disabled, or color-incapable | Existing progress appears first; after `[DONE]` a small transient inline review surface follows without alternate-screen sequences; the enhanced card shows intent, command, selected stage and purpose, focus tabs, actions, and key hints, and the plain panel is used when the card is disabled or the terminal lacks color |
