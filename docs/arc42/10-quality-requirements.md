...
- **Shell integration safety** — Optional setup, safe startup-file ownership, non-evaluating line replacement, and request preservation
  ...
  - QS-051: The widget preserves the buffer on empty, failed, or empty results
   ...
   - QS-072: Structured review responses retain model-written Stage purposes; fenced, prose-wrapped, invalid, or command-only responses keep the provider-written command reviewable with purpose-unavailable
...
| QS-072 | Shell integration / Correctness | Provider returns ready, delayed, fenced, prose-wrapped, command-only, invalid, missing-command, or stale review response data | Ready data displays exact Stage text and model-written purposes; valid delayed data displays loading then purposes; fenced or prose-wrapped data is recognized; a complete provider command stays reviewable with purpose-unavailable; a payload without a usable command shows unavailable and releases nothing; purpose text is never invented |
