...
| QS-072 | Shell integration / Correctness | Provider returns ready, delayed, fenced, prose-wrapped, unknown-status, multiline, command-only, invalid, missing-command, or stale review response data | Ready data displays exact Stage text and model-written purposes; valid delayed data displays loading then purposes; fenced or prose-wrapped data is recognized; matching stage purposes survive an unknown status word, a multiline command, and a compound provider split that provably covers the command; mismatched or missing purposes show purpose-unavailable; a payload without a usable command shows unavailable and releases nothing; purpose text is never invented |
