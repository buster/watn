...
| Review presentation | Two inline review views with optional color | The simple default shows the model short name, the stage stack, and the selected-stage purpose; the detailed view adds intent, position, and the full decision set. Both use the controlling-terminal channel, bounded dimensions, and a deterministic writer seam; never use the alternate screen or stdout |
