...
- Resolve review preference as per-invocation override over persisted configuration over default-on; a set `--review-panel`/`--no-review-panel` override persists the choice, the in-panel `d` decision persists the disable, the card is the only review surface, it opens on the command flow with direct decision keys, rejecting a candidate regenerates through the in-card model chooser, and it renders monochrome when the terminal lacks color
...
| Review presentation | Single inline review card with optional color | Uses the controlling-terminal channel, bounded dimensions, and a deterministic writer seam; never uses the alternate screen or stdout |
