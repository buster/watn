...
- Interactive setup uses native Ratatui widget composition rather than paragraph-flattened or hand-positioned terminal output; the first-run quick setup is the deliberate plain-line exception (six suggested questions, no probing)
...
- Keep the existing progress line first, then use a small transient inline review surface on the controlling-terminal channel; buffer the complete Candidate until `[DONE]` and an explicit final decision and keep review-surface bytes out of stdout
- Derive a conservative local Command flow with exact Stage text and unsupported portions tracked internally; obtain model-written Stage purposes through the structured review response and preserve reviewability with `loading` or `purpose-unavailable` status; read every review and explanation response tolerantly (repair literal control characters, recover a complete provider-written command from a malformed or truncated payload, never treat a review-shaped payload as a command), name the Purpose reason in the card, capture every unusable response in the state file, and print the raw provider response under `-v` after the surface closes
- Resolve review preference as per-invocation override over persisted configuration over default-on; a set `--review-panel`/`--no-review-panel` override persists the choice, a review switch without a request only persists the requested value, the `D` decision persists the disable while releasing the current Candidate, the card is the only review surface, it opens in a simple view with the model short name and stage stack and toggles with `d`/`?` to the detailed view, rejecting a candidate regenerates through the in-card model chooser, and it renders monochrome when the terminal lacks color
- Reuse the question path's readiness rule for `watn explain`: with no usable provider or model and no explicit provider/model selection, start quick setup (no configuration file), the setup wizard (configuration exists), or print setup guidance (non-terminal input) instead of opening the card; an explicit selection keeps its strict errors, a failed explanation request keeps the card reviewable with a visible diagnostic and the mapped exit status, and an unusable response is named on stderr
...
| Review response | Structured provider response in review mode, read tolerantly before strict validation | Carries the complete Candidate, exact Stage text, model-written Stage purposes, version, and purpose status; repaired, recovered, command-only, or invalid responses remain reviewable with `purpose-unavailable` and a named Purpose reason, and an unusable response is captured for bug reports |
| Review presentation | Two inline review views with optional color | The simple default shows the model short name with the Amount billed for the request, the stage stack, and the selected-stage purpose; the detailed view adds intent and the full decision set and carries the same Amount at its model context. Both use the controlling-terminal channel, bounded dimensions, and a deterministic writer seam; never use the alternate screen or stdout |
