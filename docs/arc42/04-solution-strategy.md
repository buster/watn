...
- Derive a conservative local Command flow with exact Stage text and unsupported portions tracked internally; obtain model-written Stage purposes through the structured review response and preserve reviewability with `loading` or `purpose-unavailable` status
