...
| Review surface boundary | Controlling-terminal channel, ANSI redraw, and review keys | Shows a small transient Command flow review without writing review-surface text to stdout; returns `Accepted(candidate)`, `Cancelled`, or `Unavailable` outcomes |
