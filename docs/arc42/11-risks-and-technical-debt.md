...
| R-092 | The Amount is absent whenever no recorded price applies to either the model the provider reported or the model the request was sent with, and that absence is silent, so a model configured without a price looks the same as a free request and the developer has no in-surface signal to fix it | Medium | Medium | Keep the silence deliberate and consistent with the stderr line, which already omits the money portion; the configuration path (`watn models`, `watn setup`) remains where a missing price is visible, and the divergence between the stderr line's `$0.0000` and the surface's silence is documented in chapter 8 |
| R-093 | The price that applies to a request falls back to the model the request was sent with when the model the provider reported has no recorded price, so a routing decision that serves a different model than the one requested is priced as the requested model | Medium | Medium | Record the routing caveat in the `observe-request-cost` rules and its example; a configuration that records prices under the provider's canonical identifiers keeps the reported model's price authoritative, and the fallback only applies where the alternative is showing nothing at all |
| R-094 | The cents form caps at four decimal places, so an amount below 0.00005 cents reads `0` even though the provider billed the request | Low | Low | The cap is the operator's chosen reading form (D2 of `amount-from-requested-model`), accepted over the previous never-zero guarantee; the stderr line keeps its `$0.0000` behaviour, and reaching the floor needs a request so small or a per-million price so low that no configured model approaches it |
| R-095 | A release range without a feature, fix, performance change, revert, or breaking change now stops the release before the version commit, so a maintenance-only release needs a manual note or a wider range | Low | Medium | The guard names the missing user-visible group; dependency updates arrive as `fix` revisions and stay included, and the maintainer can widen the range or add the missing user-visible entry before releasing |
| R-096 | The acceptance suite invokes the pinned `git-cliff` binary, so a missing or version-drifted tool fails the changelog scenarios or changes the rendered section | Low | Medium | Pin 2.13.1 in CI and the release tooling, fail the step with the exact install command when the binary is absent, and assert the observable section shape rather than internal rendering |
