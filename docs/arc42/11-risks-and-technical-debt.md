...
| R-080 | The project-owned runner can drift from the Givn 0.7.0 result contract or report counts that misclassify scenario outcomes | Medium | Medium | Counts come from the runner's own scenario statistics (`scenarios_stats()`), Givn rejects missing, malformed, failed, skipped, zero, or non-reconciling results, and both scopes are validated manually before the next archive |
