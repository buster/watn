...
| R-068 | Inline panel redraw can corrupt a wrapped prompt or leave terminal state behind | Medium | High | Use controlling-terminal cleanup on every outcome, preserve the line-editor-owned repaint boundary, cover Bash PTY acceptance/cancellation at a fixed terminal size, and fall back to the plain panel when the card is disabled or cannot render |
...
| R-071 | Enhanced renderer availability differs across terminals and multiplexers | Medium | Medium | Treat the portable inline panel as mandatory, use the colorized card only when the terminal supports it, and retry the plain panel on enhanced failure |
