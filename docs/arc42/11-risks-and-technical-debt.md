...
| R-072 | A structured review response may contain invalid, stale, or mismatched Stage purposes | Medium | Medium | Validate response version, complete Candidate command, exact Stage text, purpose status, and current Candidate identity; tolerate markdown fences and surrounding prose, recover the provider-written command from JSON-shaped payloads, show purpose-unavailable and retain reviewability on failure, and release nothing when no usable command exists |
